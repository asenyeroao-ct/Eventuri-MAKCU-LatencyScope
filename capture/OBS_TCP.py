...
class OBS_TCP_Receiver:
    """
    OBS TCP receiver for MJPEG stream from OBS Studio
    Supports receiving Motion JPEG stream over TCP protocol
    """
    
    def __init__(self, ip: str = "192.168.0.1", port: int = 1234, target_fps: int = 60, max_workers: int = None, is_server: bool = False):
        """
        Initialize OBS TCP receiver
        
        Args:
            ip: IP address to connect to (client mode) or bind to (server mode)
            port: Port number
            ...
            is_server: If True, act as server (listen for connections), else act as client (connect)
        """
        self.ip = ip
        self.port = port
        ...
        # Connection state
        self.socket = None
        self.connection = None  # Client connection (server mode)
        self.is_connected = False
        self.is_receiving = False
        
        # Threading / Async
        self.receive_thread = None
        self.accept_thread = None  # For server mode
        ...
        logger.info(f"OBS_TCP_Receiver initialized: {ip}:{port}, mode={'server' if is_server else 'client'}, max_workers={self.max_workers}")
    ...
    def connect(self) -> bool:
        """
        Connect to TCP stream (client mode) or start listening (server mode)
        ...
        try:
            ...
            if self.is_server:
                ...
                logger.info(f"Connected to TCP stream at {self.ip}:{self.port}")
            ...
            logger.error(f"Failed to connect to TCP stream: {e}")
            self.is_connected = False
            return False
        except Exception as e:
            logger.error(f"Unexpected error connecting to TCP stream: {e}")
            self.is_connected = False
            return False

    # -------------------- Async version --------------------
    class _TCPProtocol(asyncio.Protocol):
        """Asyncio protocol to receive TCP data and forward to receiver."""
        def __init__(self, receiver: "OBS_TCP_Receiver"):
        ...
        def data_received(self, data: bytes):
            ...
            try:
                ...
                logger.error(f"Protocol data handling error: {e}", exc_info=True)

        def connection_lost(self, exc):
            logger.warning(f"TCP connection lost: {exc}")
    ...
    async def _connect_asyncio(self):
        ...
        if self.is_server:
            ...
            server = await loop.create_server(
                lambda: OBS_TCP_Receiver._TCPProtocol(self),
                self.ip, self.port
            )
            self._transport = server
            await server.start_serving()
            self.is_connected = True
            logger.info(f"[async] TCP server listening on {self.ip}:{self.port}")
        else:
            # Client mode
            ...
            self.is_connected = True
            self.is_receiving = True
            logger.info(f"[async] Connected to TCP stream at {self.ip}:{self.port}")
    ...
    async def _disconnect_asyncio(self):
        try:
            ...
            logger.info("[async] Disconnected from TCP stream")
        except Exception as e:
            logger.error(f"[async] Error during disconnect: {e}", exc_info=True)

    def connect_async(self) -> bool:
        """
        Start asyncio-based TCP receiving in a background event loop thread.
        ...
        try:
            ...
            logger.error(f"Failed to start async TCP receiver: {e}", exc_info=True)
            return False

    def disconnect_async(self):
        """Stop asyncio-based TCP receiving and close transport."""
        try:
            if self._loop:
    ...
    def disconnect(self):
        ...
        try:
            ...
            logger.info("Disconnected from TCP stream")
            
        except Exception as e:
            logger.error(f"Error during disconnect: {e}", exc_info=True)
    
    def _accept_loop(self):
        """Accept connections in server mode"""
        logger.info("Started TCP accept loop")
        ...
        logger.info("TCP accept loop ended")
    
    def _receive_loop(self):
        """Main receiving loop for TCP data"""
        self.is_receiving = True
        logger.info("Started TCP receive loop")
        
        # Use connection socket if in server mode, otherwise use main socket
        sock = self.connection if self.is_server and self.connection else self.socket
        ...
        while not self.stop_event.is_set() and self.is_connected:
            try:
                # Receive TCP data (stream-based, may receive partial frames)
                data = sock.recv(65536)  # Receive up to 64KB at a time
                if not data:
                    # Connection closed
        ...
        if self.is_server:
            if self.connection:
                try:
                    ...
                    pass
                self.connection = None
        logger.info("TCP receive loop ended")
...
class OBS_TCP_Manager:
    """
    Manager class for OBS TCP connections
    Provides high-level interface for TCP stream management
    """
    ...
    def create_receiver(self, ip: str, port: int, target_fps: int = 60, is_server: bool = False) -> OBS_TCP_Receiver:
        """
        Create new TCP receiver
        
        Args:
            ip: IP address
            port: Port number
            target_fps: Target FPS
            is_server: If True, act as server, else act as client
            
        Returns:
            OBS_TCP_Receiver instance
        """
        self.receiver = OBS_TCP_Receiver(ip, port, target_fps, is_server=is_server)
        return self.receiver
    
    def connect(self, ip: str, port: int, target_fps: int = 60, is_server: bool = False) -> bool:
        """
    ...
    # -------------------- Async helpers --------------------
    def connect_async(self, ip: str, port: int, target_fps: int = 60, is_server: bool = False) -> bool:
    ...
    def get_receiver(self) -> Optional[OBS_TCP_Receiver]:
